...
Status: builder package and independent local QA complete on 2026-08-21. Checked builder/proof/QA facts are supported by current local files or the approved proof contract. Repository/draft, publish, public consumer smoke, and monitoring items remain unchecked.
...
- [ ] Public repository `main` contains the accepted package.
...
- [ ] Railway template draft exactly matches the service/port/domain/health/Serverless contract.
- [ ] Clean public consumer deploy contains exactly one app service and no hidden resource.
...
- [ ] Template editor hides generated values and does not prompt for them.
- [ ] Consumer deploy generates unique non-placeholder values.
...
- [ ] Consumer smoke rechecks public rejection of API/admin/health/metrics/debug/Swagger surfaces.

## Functional consumer smoke

- [ ] Deployment reaches terminal `SUCCESS`.
- [ ] Public `/connection/init` returns `200 {}`.
- [ ] Valid client JWT and exact Origin establish WSS.
- [ ] Wrong Origin fails.
- [ ] Malformed/expired JWT fails.
- [ ] Missing/wrong API key fails internally.
- [ ] Valid private API publish reaches the subscribed client.
- [ ] Publishing backend shares the Railway project/private network and uses private/reference wiring.
- [ ] For any sensitive/private test channel, one allowed and one denied subscription verify the chosen upstream-supported authorization mechanism.
- [ ] One restart causes disconnect, retry, reconnect, resubscribe, and new delivery.
- [ ] Client refreshes authoritative state rather than claiming memory-history recovery.
- [ ] Logs are bounded, understandable, and free of generated values.
- [ ] CPU/RAM/egress evidence is recorded without a performance or bill guarantee.
...
- [ ] Consumer restart smoke confirms new delivery without claiming pre-restart history.
...
- [ ] All repository-relative links pass after files are placed at public repo root.
...
- [x] `Other` is the fallback category pending current editor validation.
- [x] Text-only initial asset plan is documented.
- [ ] Current Railway category is selected and recorded.
- [ ] Actual generated slug matches links or all links are updated.
- [ ] Public repository/support/security links work.
- [ ] Template draft variable prompts/defaults and hidden secrets are reviewed.
- [ ] Marketplace/draft copy does not imply JWT authentication alone authorizes channels or that external backends can directly reach port `9000`.
- [ ] Concrete publish approval packet names the exact draft/listing/config.
- [ ] Publish succeeds and public page is searchable/healthy.
- [ ] Deploy button works from a clean signed-out/new-project path.
- [ ] Public consumer smoke passes before work order closure.

## Monitoring intake

- [ ] Published Templates registry row added.
- [ ] Product Kit Adoption row changed from Building to Complete.
- [ ] Published Template Monitoring Contract added.
- [ ] Template Metrics Snapshot baseline recorded.
- [ ] Support Contacts and support-health source checks recorded.
- [ ] Managed Railway Resources confirms no retained proof/source/demo resource unless intentionally owned.
- [ ] Same-day, 7-day, and 30-day review dates scheduled.
- [ ] Winner Cycle Evidence and Portfolio Operating Loop ledgers updated after consumer smoke.
- [ ] Builder, QA, and Deploy Tester registry rows closed after integration.
